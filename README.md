...
/edu-play 我想让 3 岁娃认识颜色，有什么能玩的活动？
...
| 设计活动、话术、讲法、提问、常规、沟通 | `/edu-play`、`/edu-talk`、`/edu-explain`、`/edu-question`、`/edu-routine`、`/edu-communicate` |
| 在课程 / 资源间做选择 | `/edu-decision` |
| 记录一次变化、建长期档案 | `/edu-report`、`/edu-archive` |
| 备课 / 写教案 / 写评语 | `/edu-lesson`、`/edu-comment` |
| 观察评估 / 教研反思 / 差异化 / 融合支持 | `/edu-observe`、`/edu-reflect`、`/edu-differentiate`、`/edu-inclusive` |
| 教具课件 / 环境创设 / 家园共育 / 家长课堂 | `/edu-material`、`/edu-env`、`/edu-notice`、`/edu-workshop` |
| 课堂突发与安全应急 | `/edu-safety` |
| 学习 / 阅读规划、作息屏幕、时间管理 | `/edu-studyplan`、`/edu-screen`、`/edu-time` |
| 亲子沟通 / 家庭规则与陪伴质量 | `/edu-parenttalk`、`/edu-rules` |
| 选园选校 / 幼小衔接 / 分龄指南 | `/edu-schoolchoice`、`/edu-transition`、`/edu-ages` |
| 特殊需要儿童家庭 / 父亲参与与隔代教养 | `/edu-specialhome`、`/edu-coparent` |
...
档案默认位于当前工作目录下的 `edu-档案/{孩子代号}.md`，每次更新为追加，不覆盖历史。完整规则见 [新手入门中的档案说明](docs/新手入门.md#edu-archive｜成长档案)。
